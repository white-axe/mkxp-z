...
void mkxpBlurHFS(float *_input, Shader_Builtins *builtins, void *_uniforms)
{
	...
	frag = add_vec4s(frag, mkxp_pgl_texture2D(mkxp_pgl_get_texture2D(uniforms->texture), input->v_texCoord.x, input->v_texCoord.y));
	frag = add_vec4s(frag, mkxp_pgl_texture2D(mkxp_pgl_get_texture2D(uniforms->texture), input->v_blurCoord[0].x, input->v_blurCoord[0].y));
	frag = add_vec4s(frag, mkxp_pgl_texture2D(mkxp_pgl_get_texture2D(uniforms->texture), input->v_blurCoord[1].x, input->v_blurCoord[1].y));
